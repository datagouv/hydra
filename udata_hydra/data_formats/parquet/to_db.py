...
async def parquet_to_db(
    ...
    try:
        db_cols: dict[str, str] = build_db_columns_mapping(file.inspection["columns"])
    ...
    file.inspection["columns_mapping"] = {s: pg for s, pg in db_cols.items() if s != pg}
    ...
    columns = {
        db_cols[c]: helpers.get_python_type(v) for c, v in file.inspection["columns"].items()
    }
