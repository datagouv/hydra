...
async def test_xls_analysis(setup_catalog, rmock, db, fake_check, produce_mock):
    check = await fake_check()
    filename, expected_count = ("catalog.xls", 2)
    url = check["url"]
    table_name = hashlib.md5(url.encode("utf-8")).hexdigest()
    with open(f"tests/data/{filename}", "rb") as f:
        data = f.read()
...
async def test_xlsx_analysis(setup_catalog, rmock, db, fake_check, produce_mock):
    check = await fake_check()
    filename, expected_count = ("catalog.xlsx", 2)
    url = check["url"]
    table_name = hashlib.md5(url.encode("utf-8")).hexdigest()
    with open(f"tests/data/{filename}", "rb") as f:
        data = f.read()
